...
from generate_baseline import generate, generate_with_prefix_cache, generate_with_dual_cache
...
class LLaDAEvalHarness(LM):
    def __init__(
        ...
        **kwargs,
        ...
        self.code_prompt_style = str(code_prompt_style or "raw")
        self.code_completion_postprocess = parse_bool(code_completion_postprocess)
    ...
    def generate_until(self, requests):
        ...
        for i, req in enumerate(tqdm(requests, desc="Generating...")):
            ...
            user_input = build_generation_prompt(
                ...
                code_prompt_style=self.code_prompt_style,
            )
            ...
            if self.use_cache:
                if self.dual_cache:
                    generated_answer, nfe_history = generate_with_dual_cache(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, block_length=self.block_length, 
                                        temperature=0, remasking=self.remasking, mask_id=self.mask_id, threshold=self.threshold, factor=self.factor)
                else:
                    generated_answer, nfe_history = generate_with_prefix_cache(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, block_length=self.block_length, 
                                        temperature=0, remasking=self.remasking, mask_id=self.mask_id, threshold=self.threshold, factor=self.factor)
            else:
                generated_answer, nfe_history = generate(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, block_length=self.block_length, 
                                        temperature=0, remasking=self.remasking, mask_id=self.mask_id, threshold=self.threshold, factor=self.factor)
            ...
            if should_use_raw_completion_decode(
                ...
                generated_answer = truncate_generated_text(
                    ...
                    is_instruct=self.is_instruct,
                    doc=getattr(req, "doc", None),
                    code_completion_postprocess=self.code_completion_postprocess,
                )
            else:
